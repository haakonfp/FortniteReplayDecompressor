...
public class DebuggingObject : IProperty
{
	...
	private object[] AsArray()
	{
		...
		while (true)
		{
			...
			List<DebuggingHandle> handles = new();
			bool isExportHandles = false;

			while (true)
			{
				DebuggingHandle debuggingHandle = new();
				...
				DebuggingObject obj = new();

				NetBitReader tempReader = new(_reader.ReadBits(numBits));
	...
	{
		List<IProperty> possibleProperties = new();
