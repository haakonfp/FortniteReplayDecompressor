...
public class GameInformation
{
	...
	private Dictionary<uint, uint> _actorToChannel = new();
	private Dictionary<uint, Llama> _llamas = new(); //Channel to llama
	private Dictionary<uint, SupplyDrop> _supplyDrops = new(); //Channel supply drop

	/// <summary>
	/// Requires Full parse mode
	/// </summary>
	private Dictionary<uint, InventoryItem> _floorLoot = new(); //Channel Id to InventoryItem. These will be removed/added over time

	private Dictionary<uint, Player> _players = new(); //Channel id to Player
	private Dictionary<uint, PlayerPawn> _playerPawns = new(); //Channel Id (player pawn) to Actor
	private Dictionary<uint, List<QueuedPlayerPawn>> _queuedPlayerPawns = new();
	private Dictionary<uint, Weapon> _weapons = new(); //Channel to 
	private Dictionary<uint, Weapon> _unknownWeapons = new(); //Channel to Weapon
	private Dictionary<uint, object> _containers = new(); //Channel to searchable containers
	private Dictionary<uint, PlayerStructure> _playerStructures = new(); //Channel to player structures
	private Dictionary<string, uint> _healthSetStartingHandles = new(); //Starting handle ids for a health set
	private Dictionary<uint, Vehicle> _vehicles = new(); //Channel id to vehicle
	private Dictionary<string, Player> _playerById = new(); //Used only for minigame replays
	...
	//Delta updates
	private Dictionary<uint, NetDeltaArray<PrivateTeamInfo>> _privateTeamInfo = new(); //Channel id to private team info
	private Dictionary<uint, FortInventory> _inventories = new(); //Channel to inventory items

	private Dictionary<int, Team> _teams = new();
	private List<SafeZone> _safeZones = new();
	private List<StormSurge> _stormSurges = new();
	private List<PlayerReboot> _resurrections = new();
	private List<KillFeedEntry> _killFeed = new();
	...
	internal void UpdateVehicle(uint channelId, BaseVehicle baseVehicle)
	{
		...
		{
			PlayerLocationRepMovement newLocation = new()
			{
	...
	{
		PlayerStructure newStructure = new();
